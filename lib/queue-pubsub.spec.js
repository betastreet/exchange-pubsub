...
describe('QueuePubSub', () => {
  let pubSub;
  let mockTopic;
  const pubsubDefault = {flowControl: {maxMessages: 50}};
  beforeEach(() => {
    const logMock = () => {};
  ...
  describe('subscribe', () => {
    ...
    it('should create Subscription', () => {
      ...
      return pubSub.subscribe('abc', 'sub', () => {})
        .then(() => expect(createSubscription).toHaveBeenCalledWith('sub-abc', pubsubDefault, jasmine.any(Function)));
    });
    ...
    it('should create random subscription name', () => {
      ...
      return pubSub.subscribe('abc', true, () => {})
        .then(() => expect(mockTopic.createSubscription).toHaveBeenCalledWith(jasmine.stringMatching(/^abc-.{8,}/), pubsubDefault, jasmine.any(Function)));
    });

    it('should not create random subscription name', () => {
      spyOn(mockTopic, 'createSubscription').and.callThrough();
      return pubSub.subscribe('abc', false, () => {})
        .then(() => expect(mockTopic.createSubscription).toHaveBeenCalledWith('abc', pubsubDefault, jasmine.any(Function)));
    });
    ...
    it('should handle options argument with subscription name', () => {
      ...
      return pubSub.subscribe('topic', 'please', {pubSub: options}, () => {})
        .then(() => expect(mockTopic.createSubscription).toHaveBeenCalledWith('please-topic', options, jasmine.any(Function)));
    });
    ...
    describe('on events', () => {
      ...
      describe('error', () => {
        ...
        it('should close subscription and restart on error', (done) => {
          spyOn(pubSub, 'closeSubscription');
          spyOn(pubSub, 'createAndSubscribe');
          listeners.error(new Error('test'));
          setImmediate(() => {
            expect(pubSub.closeSubscription).toHaveBeenCalledTimes(1);
            expect(pubSub.createAndSubscribe).toHaveBeenCalledTimes(1);
            done();
          });
        });
      });
  ...
  describe('subscribeOne', () => {
    ...
    it('should subscribe and close when done', (done) => {
      spyOn(pubSub, 'closeSubscription').and.returnValue(Promise.resolve());
      const promise = pubSub.subscribeOne('monkey', 'sub');
      setImmediate(() => {
        listeners.message(message);
        promise.then(msg => {
          expect(msg).toEqual('ooh ooh');
          expect(pubSub.closeSubscription).toHaveBeenCalledTimes(1);
          done();
        })
      });
  ...
  describe('closeSubscription', () => {
    it('should call close on subscription', () => {
      ...
      return pubSub.closeSubscription(sub)
        .then(() => expect(sub.close).toHaveBeenCalledTimes(1));
    });

    it('should handle errors', () => {
      const sub = {
        close: jasmine.createSpy().and.returnValue(Promise.reject(new Error('uh oh'))),
      };
      return pubSub.closeSubscription(sub)
        .then(() => {
          console.log('here');
