  ...
  setOptions(options) {
    options = defaultsDeep(options, {
      ...
      pubSub: {
        ...
        keyFilename: 'lib/gcloud-auth.json',
      },
    ...
    if (options.ignoreKeyFilename || process.env.PUBSUB_EMULATOR_HOST) {
      delete options.pubSub.keyFilename;
    }
    ...
    return createSubscription()
      .then(([subscription]) => {
        ...
        subscription.on('error', err => {
          this.log.warn('subscription error', err);
  ...
  onMessage(topicName, listener, options) {
    return (message) => {
      ...
      const handlerTimeout = options.autoAck ? setTimeout(() => {
        ...
        nack();
      }, (options.pubSub.ackDeadline || 10) * 1000) : 0;
