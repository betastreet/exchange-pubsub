...
class QueuePubSub extends QueueBase {
  ...
  setOptions(options) {
    ...
    if (!this.defaultSubscribeOptions.pubSub) {
      this.defaultSubscribeOptions.pubSub = {
        flowControl: {
          maxMessages: 50,
        },
      };
  ...
  publish(topic, obj) {
    ...
    return Promise.resolve()
      .then(() => this.createdTopics[topic] || this.createTopic(topic)
        .then(() => this.createdTopics[topic] = true))
      .then(() => publisher.publish(this.objStream(obj)))
      .catch(e => {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(msg => {
        this.closeSubscription(subscription)
          .catch(e => this.log.warn('closeSubscription error', e));
        return msg;
      });
  }
  ...
  createAndSubscribe(topic, name, options, listener) {
    ...
    return this.createTopic(topic)
      .then((psTopic) => {
        return this.createSubscription(psTopic, name, options.pubSub);
      })
      .then((subscription) => {
  ...
  createSubscription(topic, name, options) {
    const subscription = topic.subscription(name, options);
    return subscription.get({ autoCreate: true })
      .then((data) => data[0] ? data[0] : null);
  }

  closeSubscription(subscription) {
    return subscription.close()
      .catch(err => this.log.warn('subscription close error', err));
  }
  ...
  onMessage(topic, listener, options) {
    return (message) => {
      ...
      this.log.trace(`message received ${topic}`, outputArg);
