...
class QueuePubSub {
  ...
  subscribe(topicName, name, options, listener) {
    ...
    if (subOptions.subNameWithTopic && name !== topicName && name !== `${name}-${topicName}`) {
      name = `${name}-${topicName}`;
    }

    this.log.trace(`subscribing to ${topicName}`);
    const onMessage = this.onMessage(topicName, listener, subOptions);
    const resubscribeFn = (subscription) => () => {
      this.deleteSubscription(subscription)
        .then(() => this.subscribe(topicName, name, subOptions, listener))
    ...
    return Promise.resolve()
      ...
      .then(([subscription]) => {
        subscription.on('message', msg => onMessage(msg, resubscribeFn(subscription)));
        subscription.on('error', err => {
          this.log.warn(`subscription error ${topicName}`, err);
          // reSubscriber(subscription)();
        });
        return subscription;
      })
      .catch(e => {
  ...
  onMessage(topicName, listener, options) {
    return (message, resubscribeFn) => {
      ...
      const handlerTimeout = options.autoAck ? setTimeout(() => {
        ...
        options.pubSub.ackDeadline += 10;
        resubscribeFn();
      }, (Math.min(options.pubSub.ackDeadline || 10, 600) * 1000) - 500) : 0;
